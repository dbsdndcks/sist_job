...
public class MultiChatClient extends JFrame implements ActionListener,Runnable{
	...
	public void capture() throws IOException {
		File saveDir = new File("c:/E/dev/chat_backup");
		if(!saveDir.exists()) {
			saveDir.mkdir();
		}//end if
		...
		try {
			fw = new FileWriter(saveDir.getAbsolutePath() + saveFile.getAbsolutePath());
	...
	public void actionPerformed(ActionEvent ae) {
		if(ae.getSource() == jbtnConnectServer) { //서버 연결
			try {
				...
				JOptionPane.showMessageDialog(this, "서버와 연결이 종료됨");
				e.printStackTrace();
			}
		}
		if(ae.getSource() == jtfTalk) { // 대화입력
			try {
				sendMsg();
			} catch (IOException e) {
				e.printStackTrace();
