		...
		$.ajax({
			...
			success:function(xmlObj){
				...
				output+="<td style='width:300px'>제목</td><td>"+
						$(itemNode).find("title").text()+"</td>";	
				output+="<td style='width:150px'>기사작성일</td><td>"+
						$(itemNode).find("pubDate").text()+"</td>";	
				output+="</tr>"
				output+="<tr>"
				output+="<td style='width:80px'>내용</td><td colspan='5'>"+
...
%>	
https://fs.jtbc.co.kr/RSS/
<table>
<tr>
<%
for(int i=0; i<title.length; i++){
%>
<td><a href="#void" onclick="xmlParsing('<%= url[i]%>')"><%= title[i] %></a></td>
<%
}
%>
</tr>
